...
export const makePaymentFx = createEffect(async ({ url, amount, description }: IMakePayFx) => {
  const { data } = await api.post(url, { amount, description });

  return data;
});

export const checkPaymentFx = createEffect(async ({ url, paymentId }: ICheckPayFx) => {
