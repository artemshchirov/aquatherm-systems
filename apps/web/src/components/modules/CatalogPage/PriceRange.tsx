...
const PriceRange = ({ priceRange, setPriceRange, setIsPriceRangeChanged }: IPriceRangeProps) => {
  ...
  const handlePriceRangeChange = (prices: number[]) => {
  ...
  return (
    <div className={styles.filters__price}>
      <div className={`${styles.filters__price__inputs} ${darkModeClass}`}>
        <input type="text" value={Math.ceil(priceRange[0])} placeholder={`from ${MIN}`} readOnly />
        <span className={styles.filters__price__inputs__border} />
        <input type="text" value={Math.ceil(priceRange[1])} placeholder={`from ${MAX}`} readOnly />
      </div>
      <Range
        values={priceRange}
        step={STEP}
