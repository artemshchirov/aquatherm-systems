...
const CatalogPage = ({ query }: { query: IQueryParams }) => {
    ...
    try {
      ...
      const result = await getProductsFx(
        `products?limit=20&offset=${selected}${
          isFilterInQuery && isValidVendorQuery ? `&vendor=${params.get('vendor')}` : ''
        }${isFilterInQuery && isValidCategoryQuery ? `&category=${params.get('category')}` : ''}${
  ...
  const validPageCount = Number.isInteger(pagesCount) ? Math.ceil(pagesCount) : 0;
      ...
      <div className={`container ${styles.catalog__container}`}>
        ...
        <div className={`${styles.catalog__top} ${darkModeClass}`}>
          ...
          <div className={styles.catalog__top__inner}>
            ...
            <button className={styles.catalog__top__mobile_btn} onClick={toggleOpen}>
              ...
              <span className={styles.catalog__top__mobile_btn__text}>Filter</span>
            </button>
        ...
        <div className={styles.catalog__bottom}>
            ...
            {spinner ? (
              ...
              <ul className={styles.catalog__list}>
                {products.rows?.length ? (
                  products.rows.map(item => <CatalogItem item={item} key={item.id} />)
                ) : (
          ...
          <ReactPaginate
            ...
            forcePage={currentPage >= validPageCount ? validPageCount - 1 : currentPage}
