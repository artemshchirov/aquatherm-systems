...
export const categories = ['Sets', 'Paints', 'Canvas'].map(createManufacturerCheckboxObj);
...
export const checkQueryParams = (params: URLSearchParams, path: string) => {
  const priceFromQueryValue = getQueryParamOnFirstRender('priceFrom', params, path) as string;
  ...
  const isValidPriceQuery =
    checkPriceFromQuery(+priceFromQueryValue) && checkPriceFromQuery(+priceToQueryValue);
